...
(function(){
  ...
  const ICONS={
    profile:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 21a8 8 0 0 0-16 0"></path><circle cx="12" cy="8" r="4"></circle></svg>',
    ...
    access:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="14" rx="2"></rect><path d="M7 20h10"></path><path d="M12 17v3"></path></svg>',
    orders:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 2h12l1 4H5l1-4Z"></path><path d="M4 6h16v5a2 2 0 0 1-2 2h-1"></path><path d="M4 6v11a2 2 0 0 0 2 2h5"></path><path d="M14 17l2 2 4-4"></path></svg>',
    login:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 3h3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-3"></path><path d="M10 17l5-5-5-5"></path><path d="M15 12H3"></path></svg>',
    logout:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 3h3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-3"></path><path d="M10 17l-5-5 5-5"></path><path d="M5 12h12"></path></svg>'
  };
  ...
  function menuType(el){
    const href=(el.getAttribute('href')||'').toLowerCase();
    const id=(el.id||'').toLowerCase();
    const text=normalizeLabel(el.textContent).toLowerCase();
    if(href.includes('profile.html')||text.includes('edit profile')) return 'edit';
    if(href.includes('products-access')||text.includes('products')||text.includes('access')) return 'access';
    if(href.includes('orders')||text.includes('orders')) return 'orders';
    if(id.includes('logout')||text.includes('log out')) return 'logout';
    if(id.includes('login')||text.includes('sign in')||text.includes('login')) return 'login';
    return 'profile';
  }

  function cleanedMenuLabel(type, fallback){
    return {
      profile:'View profile',
      edit:'Edit profile information',
      access:'My products / access',
      orders:'My orders',
      login:'Sign in/up',
      logout:'Log out'
    }[type] || normalizeLabel(fallback);
  }

  function decorateMenuItem(el){
    if(!el) return;
    const type=menuType(el);
    const label=cleanedMenuLabel(type, el.textContent);
    if(el.dataset.fbDecorated==='1'){
      const labelNode=el.querySelector('.fb-menu-label');
      if(labelNode) labelNode.textContent=label;
      return;
    }
    el.dataset.fbDecorated='1';
    el.setAttribute('aria-label',label);
    el.innerHTML='<span class="fb-menu-icon" aria-hidden="true">'+ICONS[type]+'</span><span class="fb-menu-label">'+label+'</span>';
  }

  function decorateProfileMenu(){
    const menu=$('profileMenu');
    if(!menu) return;
    $$('#profileMenu button,#profileMenu a.profile-menu-link').forEach(decorateMenuItem);
  }

  function polishGeneratorCopy(){
    const modalTitle=document.querySelector('#fitbrandGeneratorModal .fb-modal-header h2');
    const modalText=document.querySelector('#fitbrandGeneratorModal .fb-modal-header p');
    if(modalTitle) modalTitle.textContent='Generate your personal training plan';
    if(modalText) modalText.textContent='Fill in your details and FitBrand will create a cleaner, more personal plan based on the package you bought.';
    const mealHead=document.querySelector('#meal-plan-ai .meal-ai-lock-head p');
    if(mealHead) mealHead.textContent='Answer a few simple questions and get your custom FitBrand meal plan in seconds.';
  }
  ...
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', bootV16);
  else bootV16();

  const observer=new MutationObserver(()=>bootV16());
  const startObserver=()=>{ if(document.body) observer.observe(document.body,{childList:true,subtree:true}); };
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', startObserver);
  else startObserver();
  setInterval(bootV16, 1200);
})();

/* ===== FITBRAND v17 PREMIUM EXPERIENCE PATCH ===== */
(function(){
  'use strict';
  const $=id=>document.getElementById(id);
  const $$=(sel,root=document)=>Array.from(root.querySelectorAll(sel));
  ...
  function premiumBundleUpgrade(){
    const box=document.querySelector('.fitbrand-bundle-offer');
    if(!box || box.dataset.v17==='1') return;
    box.dataset.v17='1';
    const mainText=box.querySelector('p:nth-of-type(2), p:last-of-type');
    if(mainText){
      mainText.textContent='Get every FitBrand training program plus the Meal Plan Guide AI in one premium system. Built for customers who want the full coaching-style experience, better results and the best overall value.';
    }
    if(!box.querySelector('.fb-bundle-micro')){
      const micro=document.createElement('div');
      micro.className='fb-bundle-micro';
      micro.innerHTML='<span>Instant access</span><span>Meal Plan AI included</span><span>Training generator included</span><span>Best value option</span>';
      const actions=box.querySelector('.bundle-btn-row,.finder-actions,.bundle-actions');
      if(actions) actions.before(micro); else box.appendChild(micro);
    }
  }

  function polishHeadings(){
    $$('h1,h2,h3').forEach(el=>{
      if(el.closest('.profile-menu')) return;
...
/* ===== FITBRAND v18 CUSTOMER READY AUTH / ACCESS / AI FINAL OVERRIDE ===== */
(function(){
  'use strict';
  const USER='fitbrandUser';
  const SESSION='fitbrandSessionUser';
  const CART='fitbrandCart';
  const PUR='fitbrandPurchases';
  const ORD='fitbrandOrders';
  const PROG='fitbrandPurchasedPackage';
  const MEAL='fitbrandMealPlanUnlocked';
  const $=id=>document.getElementById(id);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const productData={
    aesthetic:{name:'Aesthetic Program',price:4.99,type:'digital',url:'index.html?purchased=aesthetic',program:true},
    shred:{name:'Shred Program',price:6.99,type:'digital',url:'index.html?purchased=shred',program:true},
    strength:{name:'Strength Program',price:6.99,type:'digital',url:'index.html?purchased=strength',program:true},
    bundle:{name:'Complete Bundle + Meal Plan AI',price:18.97,type:'digital',url:'index.html?purchased=bundle',program:true},
    mealplan:{name:'Meal Plan Guide AI',price:5.99,type:'digital',url:'recommended.html?purchased=mealplan#meal-plan-ai'},
    belt:{name:'Lifting Belt',price:24.99,type:'comingsoon'},
    straps:{name:'Lifting Straps',price:12.99,type:'comingsoon'}
  };
  const valid=Object.keys(productData);
  const programs=['aesthetic','shred','strength'];
  ...
  function getUser(){return parse(localStorage.getItem(USER),null)||parse(sessionStorage.getItem(SESSION),null)}
  function displayNameFromEmail(email){return String(email||'').split('@')[0].replace(/[._-]+/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}
  function initials(u){
    if(!u) return '?';
    const base=String(u.name||displayNameFromEmail(u.email)||'').trim();
    const parts=base.split(/\s+/).filter(Boolean);
    if(parts.length>=2) return (parts[0][0]+parts[parts.length-1][0]).toUpperCase();
    const emailParts=String(u.email||'').split('@')[0].replace(/[._-]+/g,' ').trim().split(/\s+/).filter(Boolean);
    if(emailParts.length>=2) return (emailParts[0][0]+emailParts[emailParts.length-1][0]).toUpperCase();
    return base.slice(0,2).toUpperCase() || '?';
  }
  function rawPurchases(){
    let arr=parse(localStorage.getItem(PUR),[]).filter(x=>valid.includes(x));
    const p=localStorage.getItem(PROG);
    if(valid.includes(p) && !arr.includes(p)) arr.push(p);
    if(localStorage.getItem(MEAL)==='true' && !arr.includes('mealplan')) arr.push('mealplan');
    if(arr.includes('bundle')){
      ['aesthetic','shred','strength','mealplan'].forEach(x=>{if(!arr.includes(x))arr.push(x)});
    }
    return [...new Set(arr)];
  }
  function purchases(){return getUser()?rawPurchases():[]}
  function saveUser(data,remember){
    const old=getUser()||{};
    const u=Object.assign({},old,data||{});
    if(!u.email || !emailOk(u.email)){alert('Please enter a valid email.');return false}
    if(!u.name) u.name=displayNameFromEmail(u.email);
    if(remember){localStorage.setItem(USER,JSON.stringify(u));sessionStorage.removeItem(SESSION)}
    else{sessionStorage.setItem(SESSION,JSON.stringify(u));localStorage.removeItem(USER)}
  ...
  function icon(name){
    const icons={
      ...
      access:'<svg viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"></path><path d="M3.3 7 12 12l8.7-5"></path><path d="M12 22V12"></path></svg>',
      orders:'<svg viewBox="0 0 24 24"><path d="M6 2h12l1 4H5l1-4Z"></path><path d="M4 6h16v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2Z"></path><path d="M9 12h6"></path><path d="M9 16h4"></path></svg>',
      login:'<svg viewBox="0 0 24 24"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"></path><path d="M10 17l5-5-5-5"></path><path d="M15 12H3"></path></svg>',
    ...
    return '<span class="fb-menu-icon">'+icons[name]+'</span>';
  }
  function ensureProfile(){
    ...
    let menu=$('profileMenu');
    if(!menu){menu=document.createElement('div');menu.id='profileMenu';menu.className='profile-menu';actions.appendChild(menu)}
    menu.innerHTML='<div class="profile-menu-head"><div class="profile-avatar"><span id="profileMenuInitial">?</span></div><div class="profile-menu-id"><strong id="profileMenuName">Guest</strong><span id="profileMenuEmail">Not logged in</span></div></div>'+
      '<button type="button" data-auth-only="1" onclick="openProfileModal(\'profile\')">'+icon('profile')+'<span>View profile</span></button>'+
      '<a class="profile-menu-link" data-auth-only="1" href="profile.html">'+icon('edit')+'<span>Edit profile information</span></a>'+
      '<a class="profile-menu-link" data-auth-only="1" href="products-access.html">'+icon('access')+'<span>My products / access</span></a>'+
      '<a class="profile-menu-link" data-auth-only="1" href="orders.html">'+icon('orders')+'<span>My orders</span></a>'+
      '<button type="button" id="profileLoginBtn" data-guest-only="1" onclick="openProfileModal(\'login\')">'+icon('login')+'<span>Sign in/up</span></button>'+
      '<button type="button" id="profileLogoutBtn" data-auth-only="1" onclick="logoutFitBrandUser()">'+icon('logout')+'<span>Log out</span></button>';
    if(!$('profileModal')){
      document.body.insertAdjacentHTML('beforeend','<div id="profileModal" class="profile-modal-overlay" onclick="closeProfileModal()"><div class="profile-modal" onclick="event.stopPropagation()"><button class="profile-modal-close" type="button" onclick="closeProfileModal()">×</button><div class="profile-modal-head"><div class="profile-avatar large"><span id="profileModalInitial">?</span></div><h2 id="profileModalTitle">Sign in/up</h2><p id="profileModalSubtitle">Save your orders, product access and profile details on this device.</p></div><div id="profileView" class="profile-modal-content" style="display:none"><div class="profile-info-box"><strong>Name</strong><span id="profileViewName">Guest</span></div><div class="profile-info-box"><strong>Email</strong><span id="profileViewEmail">Not logged in</span></div><div class="profile-info-box"><strong>Status</strong><span id="profileViewStatus">Guest mode</span></div></div><div id="profileLogin" class="profile-modal-content"><label>Email</label><input id="loginProfileEmail" type="email" placeholder="your@email.com"><label>Name</label><input id="loginProfileName" type="text" placeholder="Your full name"><label class="remember-row"><input id="rememberLoginCheck" type="checkbox" checked><span>Remember my login</span></label><button class="profile-main-btn" type="button" onclick="loginFitBrandUser()">Sign in/up</button><p class="profile-small-note">Demo login saves locally on this device.</p></div></div></div>');
    }
  }
  function updateProfileUI(){
    ...
    $$('[data-auth-only]').forEach(el=>el.style.display=u?'flex':'none');
    $$('[data-guest-only],#profileLoginBtn').forEach(el=>el.style.display=u?'none':'flex');
    $$('#profileLogoutBtn').forEach(el=>el.style.display=u?'flex':'none');
    renderAccountPages();
    gatePremiumTools();
  }
  function toggleProfileMenu(){ensureProfile();$('profileMenu')?.classList.toggle('show');updateProfileUI()}
  function openProfileModal(mode){
    ensureProfile();
    $('profileMenu')?.classList.remove('show');
    const modal=$('profileModal'), login=$('profileLogin'), view=$('profileView');
    if(!modal) return;
    const u=getUser();
    if(mode==='profile' && u){
      $('profileModalTitle').textContent='Your profile';
      $('profileModalSubtitle').textContent='Your saved FitBrand profile and customer status.';
      if(view) view.style.display='block'; if(login) login.style.display='none';
    }else{
      $('profileModalTitle').textContent='Sign in/up';
      $('profileModalSubtitle').textContent='Use your email so orders and product access connect to your profile.';
      if(view) view.style.display='none'; if(login) login.style.display='block';
      if($('loginProfileEmail')) $('loginProfileEmail').value=u?.email||'';
      if($('loginProfileName')) $('loginProfileName').value=u?.name||'';
    }
    updateProfileUI();
    modal.classList.add('show');document.body.classList.add('fb-modal-open');
  }
  function closeProfileModal(){ $('profileModal')?.classList.remove('show'); document.body.classList.remove('fb-modal-open'); }
  function loginFitBrandUser(){
    const email=($('loginProfileEmail')?.value||'').trim();
    const name=($('loginProfileName')?.value||'').trim() || displayNameFromEmail(email);
    const remember=!!$('rememberLoginCheck')?.checked;
    if(!emailOk(email)){alert('Please enter a valid email.');return}
    saveUser({email,name},remember);
    closeProfileModal();
    if(sessionStorage.getItem('fbAfterLoginRedirect')){const u=sessionStorage.getItem('fbAfterLoginRedirect');sessionStorage.removeItem('fbAfterLoginRedirect');location.href=u;}
  }
  function showWelcome(){
    if(getUser() || sessionStorage.getItem('fitbrandWelcomeDone')==='1') return;
    if(!$('fbWelcomeOverlay')){
      document.body.insertAdjacentHTML('beforeend','<div id="fbWelcomeOverlay" class="fb-welcome-overlay"><div class="fb-welcome-card"><div class="profile-avatar large"><span>FB</span></div><h2>Welcome to FitBrand</h2><p>Sign in to save your profile, orders and product access, or continue as guest and browse first.</p><div class="cart-actions"><button class="btn-dark" type="button" id="fbWelcomeLogin">Sign in/up</button><button class="btn-outline" type="button" id="fbWelcomeGuest">Continue as guest</button></div></div></div>');
      $('fbWelcomeLogin').onclick=()=>{sessionStorage.setItem('fitbrandWelcomeDone','1');$('fbWelcomeOverlay').classList.remove('show');openProfileModal('login')};
      $('fbWelcomeGuest').onclick=()=>{sessionStorage.setItem('fitbrandWelcomeDone','1');$('fbWelcomeOverlay').classList.remove('show');document.body.classList.remove('fb-modal-open')};
    }
    setTimeout(()=>{if(!getUser()&&sessionStorage.getItem('fitbrandWelcomeDone')!=='1'){$('fbWelcomeOverlay')?.classList.add('show');document.body.classList.add('fb-modal-open')}},450);
  }
  function cart(){return parse(localStorage.getItem(CART),[]).filter(k=>valid.includes(k)&&productData[k].type!=='comingsoon')}
  function saveCart(c){localStorage.setItem(CART,JSON.stringify((c||[]).filter(k=>valid.includes(k)&&productData[k].type!=='comingsoon')))}
  function addToCart(k){
    if(!valid.includes(k)) return;
    if(productData[k].type==='comingsoon'){alert(productData[k].name+' is coming soon.');return}
    const c=cart();c.push(k);saveCart(c);updateCartCount();showMiniCartPopup();
  }
  function updateCartCount(){
    const c=cart();
    $$('#cart-count,#cart-count-btn').forEach(el=>{el.textContent=c.length;el.style.display=c.length?'inline-flex':'none'});
    renderDrawer();
  }
  function renderDrawer(){
    const items=$('drawer-items'), total=$('drawer-total'); if(!items||!total)return;
    const c=cart(); if(!c.length){items.innerHTML='<p>Your cart is empty.</p>';total.textContent='€0.00';return}
    let sum=0;items.innerHTML=c.map((k,i)=>{const p=productData[k]||productData.bundle;sum+=p.price;return '<div class="drawer-item"><div><strong>'+esc(p.name)+'</strong><span>FitBrand product</span></div><div><strong>'+money(p.price)+'</strong><br><button class="remove-item-btn" onclick="removeDrawerItem('+i+')">Remove</button></div></div>'}).join('');
    total.textContent=money(sum);
  }
  function removeDrawerItem(i){const c=cart();c.splice(i,1);saveCart(c);updateCartCount()}
  function openCartDrawer(){$('cart-drawer')?.classList.add('show');$('drawer-overlay')?.classList.add('show');renderDrawer()}
  function closeCartDrawer(){$('cart-drawer')?.classList.remove('show');$('drawer-overlay')?.classList.remove('show')}
  function showMiniCartPopup(){const p=$('mini-cart-popup'); if(!p)return; p.classList.add('show'); setTimeout(()=>p.classList.remove('show'),2600)}
  function recordDemoPurchase(keys,email){
    keys=[...new Set(keys.filter(k=>valid.includes(k)&&productData[k].type!=='comingsoon'))];
    let arr=parse(localStorage.getItem(PUR),[]).filter(k=>valid.includes(k));
    keys.forEach(k=>{if(!arr.includes(k))arr.push(k)});
    if(arr.includes('bundle')) ['aesthetic','shred','strength','mealplan'].forEach(k=>{if(!arr.includes(k))arr.push(k)});
    localStorage.setItem(PUR,JSON.stringify(arr));
    if(arr.includes('mealplan')) localStorage.setItem(MEAL,'true');
    const order={date:new Date().toLocaleString(),items:keys,total:keys.reduce((s,k)=>s+(productData[k]?.price||0),0),email:email||getUser()?.email||''};
    const orders=parse(localStorage.getItem(ORD),[]);orders.unshift(order);localStorage.setItem(ORD,JSON.stringify(orders.slice(0,30)));
    if(email && emailOk(email) && !getUser()) saveUser({email,name:displayNameFromEmail(email)},true);
  }
  function checkoutItems(){
    const q=new URLSearchParams(location.search);
    if(q.get('cart')==='true') return cart();
    const p=q.get('product')||'bundle';
    return valid.includes(p)?[p]:['bundle'];
  }
  function enhanceCheckout(){
    if(!location.pathname.endsWith('checkout.html')) return;
    const pay=$('stripe-link'), email=$('checkout-email'), policies=$('accept-policies'); if(!pay||!email||!policies)return;
    function update(){
      const ok=emailOk(email.value)&&policies.checked;
      pay.classList.toggle('btn-disabled',!ok);
      pay.href=ok?'confirmation.html?demo=true&items='+encodeURIComponent(checkoutItems().join(','))+'&email='+encodeURIComponent(email.value.trim()):'#';
      const msg=$('checkout-email-message'); if(msg) msg.textContent=emailOk(email.value)?'Email ready. Your FitBrand account will be saved with this email.':'Email is required before payment. We use it to create/save your FitBrand account on this device.';
    }
    email.addEventListener('input',update);policies.addEventListener('change',update);
    pay.onclick=function(e){
      if(!emailOk(email.value)){e.preventDefault();alert('Please enter your email first.');return}
      if(!policies.checked){e.preventDefault();alert('Please accept the policies first.');return}
      saveUser({email:email.value.trim(),name:getUser()?.name||displayNameFromEmail(email.value)},true);
    };
    update();
  }
  function processConfirmation(){
    if(!location.pathname.endsWith('confirmation.html')) return;
    const q=new URLSearchParams(location.search);
    const items=(q.get('items')||q.get('product')||q.get('purchased')||'').split(',').filter(Boolean);
    const email=q.get('email');
    if(items.length) recordDemoPurchase(items,email);
    saveCart([]);updateCartCount();updateProfileUI();
  }
  function renderAccountPages(){
    const u=getUser();
    const locked=$('accountLocked'), intro=$('accountIntro');
    const access=$('accessGrid'), orders=$('ordersList');
    if(locked) locked.style.display=u?'none':'grid';
    if(intro) intro.textContent=u?'Logged in as '+(u.email||u.name)+'.':'Log in to see your saved access and orders on this device.';
    if(access){
      if(!u){access.innerHTML='';return}
      const own=purchases();
      access.innerHTML=['aesthetic','shred','strength','bundle','mealplan'].map(k=>{
        const p=productData[k], has=own.includes(k)||own.includes('bundle');
        return '<article class="access-card '+(has?'owned':'')+'"><span>'+(has?'Unlocked':'Locked')+'</span><h3>'+esc(p.name)+'</h3><p>'+(has?'You have access to this product on this device.':'Buy this product to unlock access.')+'</p><a class="'+(has?'btn-dark':'btn-outline')+'" href="'+(has?p.url:'checkout.html?product='+k)+'">'+(has?'Open':'Buy access')+'</a></article>';
      }).join('');
    }
    if(orders){
      if(!u){orders.innerHTML='';return}
      const list=parse(localStorage.getItem(ORD),[]);
      orders.innerHTML=list.length?list.map(o=>'<article class="order-card"><div><strong>'+esc(o.date)+'</strong><span>'+esc((o.items||[]).map(k=>productData[k]?.name||k).join(', '))+'</span></div><b>'+money(o.total)+'</b></article>').join(''):'<p>No saved demo orders yet.</p>';
    }
  }
  function gatePremiumTools(){
    const u=getUser(); const own=purchases();
    const canTrain=u && own.some(k=>programs.includes(k)||k==='bundle');
    if($('openGeneratorBtn')) $('openGeneratorBtn').classList.toggle('show',!!canTrain);
    const meal=$('meal-plan-ai');
    if(meal){
      const canMeal=u && (own.includes('mealplan')||own.includes('bundle'));
      meal.classList.toggle('unlocked',!!canMeal);
      if(!canMeal) meal.classList.remove('show');
    }
  }
  ...
  function validateAge(value){const age=Number(value||0);return !age||age>=16}
  function ageAlert(){alert('You must be at least 16 years old to use FitBrand AI generators. If you are under 16, you need permission from a parent or guardian.');}
  const trainingOptions={
    aesthetic:{title:'Aesthetic AI',goals:[['upper','Upper-body shape','Focus on chest, shoulders and back width.'],['balanced','Balanced physique','Proportions, symmetry and clean weekly structure.'],['leanmuscle','Lean muscle','Build muscle while staying athletic.']]},
    shred:{title:'Shred AI',goals:[['fatloss','Fat loss','Keep strength while cutting body fat.'],['definition','Muscle definition','More conditioning and visible shape.'],['discipline','Consistency system','Simple plan with steps and structure.']]},
    strength:{title:'Strength AI',goals:[['squatbenchdead','Main lifts','Improve squat, bench and deadlift.'],['power','Power focus','Heavy compounds and performance.'],['muscle-strength','Strength + size','Strong lifts with hypertrophy accessories.']]},
    bundle:{title:'Complete Bundle AI',goals:[['fullsystem','Full system','Training plus nutrition structure.'],['transform','Transformation','Build muscle, lose fat and stay consistent.'],['longterm','Long-term progress','Rotate programs with structured phases.']]}
  };
  function currentProgramAccess(){
    const q=new URLSearchParams(location.search); let p=q.get('purchased')||localStorage.getItem(PROG)||'';
    const own=purchases();
    if(own.includes('bundle')) return 'bundle';
    if(programs.includes(p)&&own.includes(p)) return p;
    return own.find(k=>programs.includes(k))||'';
  }
  function openTrainingGenerator(){
    if(!getUser()){sessionStorage.setItem('fbAfterLoginRedirect','index.html');openProfileModal('login');return}
    const pack=currentProgramAccess();
    if(!pack){alert('Buy a FitBrand program first to unlock the training generator.');return}
    let modal=$('fitbrandGeneratorModal');
    if(!modal){document.body.insertAdjacentHTML('beforeend','<div id="fitbrandGeneratorModal" class="fb-modal-overlay"><div class="fb-generator-modal"><button class="fb-modal-close" onclick="closeGeneratorModal()">×</button><div class="fb-modal-header"><div class="fb-logo-mark">FB</div><div class="fb-modal-kicker">After purchase</div><h2>Generate Your Personal Training Plan</h2><p>Premium step-by-step training plan based on the program you own.</p></div><div class="fb-modal-body" id="fbTrainingModalBody"></div></div></div>');modal=$('fitbrandGeneratorModal')}
    const body=$('fbTrainingModalBody')||modal.querySelector('.fb-modal-body');
    const u=getUser()||{}, opt=trainingOptions[pack]||trainingOptions.aesthetic;
    body.innerHTML='<div id="trainingWizard" class="fb-premium-ai-wizard"><div class="fb-ai-top"><div><span>'+esc(opt.title)+'</span><h3>Build your plan</h3><p>This generator is matched to your purchased program, so each product has its own goal options.</p></div><strong id="trainStepPill">1 / 4</strong></div><div class="fb-ai-progress"><i id="trainProgressFill" style="width:25%"></i></div><section class="fb-ai-step active" data-step="1"><h4>Choose your focus</h4><div class="fb-program-track-options">'+opt.goals.map(g=>'<button data-field="focus" data-value="'+g[0]+'"><strong>'+g[1]+'</strong><small>'+g[2]+'</small></button>').join('')+'</div></section><section class="fb-ai-step" data-step="2"><h4>Your information</h4><div class="fb-ai-profile-card"><div><strong>'+esc(u.name||'Profile info')+'</strong><span>'+esc((u.age||'No age')+' years • '+(u.weight||'No weight')+' kg • '+(u.height||'No height')+' cm')+'</span></div><a href="profile.html">Change information</a></div><div class="fb-ai-form-grid"><input id="twAge" type="number" placeholder="Age" value="'+esc(u.age||'')+'"><select id="twGender"><option value="">Gender</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select><input id="twWeight" type="number" placeholder="Weight kg" value="'+esc(u.weight||'')+'"><input id="twHeight" type="number" placeholder="Height cm" value="'+esc(u.height||'')+'"></div><div id="twAgeWarning" class="fb-age-warning">Minimum age is 16.</div></section><section class="fb-ai-step" data-step="3"><h4>Training setup</h4><div class="fb-ai-form-grid"><select id="twPlace"><option value="gym">Gym</option><option value="home">Home</option></select><select id="twDays"><option value="3">3 days/week</option><option value="4">4 days/week</option><option value="5">5 days/week</option><option value="6">6 days/week</option></select><select id="twLevel"><option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option></select><select id="twLimit"><option value="none">No limitation</option><option value="knees">Knee discomfort</option><option value="shoulders">Shoulder discomfort</option><option value="time">Short workouts</option></select></div></section><section class="fb-ai-step" data-step="4"><h4>Generate your plan</h4><p>We will create a plan that matches your purchased '+esc(productData[pack].name)+'.</p><button class="fb-ai-generate" id="twGenerate">Generate training plan</button></section><div class="fb-ai-actions"><button id="twBack">Back</button><button id="twNext">Next</button></div><div id="modalPlanOutput" class="fb-plan-output"><div class="fb-plan-head"><div><h3 id="modalPlanTitle">Your plan</h3><p id="modalPlanSubtitle"></p></div><div id="modalPlanPill" class="fb-plan-pill"></div></div><div id="modalPlanDays" class="fb-plan-days"></div><div class="meal-action-row"><button class="download-btn" onclick="downloadTrainingPlanPDF()">Download PDF</button><button class="reset-btn" onclick="downloadTrainingPlanPNG()">Download PNG</button></div></div></div>';
    if($('twGender')) $('twGender').value=u.gender||''; if($('twPlace')) $('twPlace').value=u.trainingLocation||'gym'; if($('twDays')) $('twDays').value=u.trainingDays||'4'; if($('twLevel')) $('twLevel').value=u.level||'beginner';
    let step=1, focus=opt.goals[0][0]; const daysBase={aesthetic:['Chest + Shoulders','Back Width','Legs + Core','Upper Aesthetic','Pump + Arms','Weak Points'],shred:['Full Body Strength','Conditioning','Lower + Cardio','Upper + HIIT','Fat Loss Circuit','Active Recovery'],strength:['Squat Focus','Bench Focus','Deadlift Focus','Overhead Press','Volume Strength','Recovery Strength'],bundle:['Aesthetic Upper','Shred Conditioning','Strength Lower','Meal Prep + Mobility','Full Body Progress','Recovery + Steps']};
    function render(){ $$('#trainingWizard .fb-ai-step').forEach(s=>s.classList.toggle('active',Number(s.dataset.step)===step)); $('trainStepPill').textContent=step+' / 4'; $('trainProgressFill').style.width=(step/4*100)+'%'; $('twBack').style.visibility=step===1?'hidden':'visible'; $('twNext').style.display=step===4?'none':'inline-flex'; }
    $('trainingWizard').onclick=e=>{const b=e.target.closest('button'); if(!b)return; if(b.dataset.field==='focus'){focus=b.dataset.value; $$('[data-field="focus"]').forEach(x=>x.classList.toggle('selected',x===b));} if(b.id==='twBack'&&step>1){step--;render()} if(b.id==='twNext'&&step<4){if(step===2&&!validateAge($('twAge').value)){$('twAgeWarning')?.classList.add('show');ageAlert();return}step++;render()} if(b.id==='twGenerate'){if(!validateAge($('twAge').value)){ageAlert();return}generateTraining(pack,focus,daysBase[pack]||daysBase.aesthetic)}};
    $$('[data-field="focus"]')[0]?.classList.add('selected');render();
    modal.classList.add('show');document.body.classList.add('fb-modal-open');
  }
  function generateTraining(pack,focus,baseDays){
    const days=Number($('twDays')?.value||4), place=$('twPlace')?.value||'gym', level=$('twLevel')?.value||'beginner';
    const exerciseMap={gym:['Compound lift — 4x6-8','Main accessory — 3x8-10','Machine/control work — 3x10-12','Isolation finisher — 3x12-15','Core or cardio — 10 min'],home:['Main bodyweight/DB lift — 4x10','Single-leg or push/pull accessory — 3x10','Band/DB control work — 3x12','Core finisher — 3 rounds','Walk or mobility — 10 min']};
    const chosen=baseDays.slice(0,days);
    $('modalPlanTitle').textContent='Your '+productData[pack].name+' Plan';
    $('modalPlanSubtitle').textContent='Built for '+level+' level, '+place+' training, '+days+' days per week. Focus: '+focus+'.';
    $('modalPlanPill').textContent=days+' DAYS / '+place.toUpperCase();
    $('modalPlanDays').innerHTML=chosen.map((d,i)=>'<div class="fb-plan-day"><h4>Day '+(i+1)+' — '+esc(d)+'</h4><ul>'+exerciseMap[place].map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul></div>').join('');
    window.latestTrainingPlanText='FITBRAND TRAINING PLAN\n\nPackage: '+productData[pack].name+'\nFocus: '+focus+'\nPlace: '+place+'\nDays: '+days+'\n\n'+chosen.map((d,i)=>'Day '+(i+1)+' - '+d+'\n'+exerciseMap[place].map(x=>'- '+x).join('\n')).join('\n\n');
  ...
  function handleMealPreviewClick(){
    if(!getUser()){openProfileModal('login');return}
    if(!(purchases().includes('mealplan')||purchases().includes('bundle'))){document.querySelector('a[href="checkout.html?product=mealplan"]')?.scrollIntoView({behavior:'smooth',block:'center'});alert('Buy Meal Plan Guide AI to unlock the generator.');return}
    buildMealWizard();$('meal-plan-ai')?.classList.add('unlocked');$('meal-plan-ai')?.scrollIntoView({behavior:'smooth',block:'start'});
  }
  function buildMealWizard(){
    const gen=$('mealGenerator'); if(!gen||$('fitbrandMealWizardV18')) return;
    gen.querySelector('.meal-grid')?.classList.add('fb-hidden-original'); gen.querySelector('.meal-generate-btn')?.classList.add('fb-hidden-original');
    const u=getUser()||{}; const wiz=document.createElement('div'); wiz.id='fitbrandMealWizardV18'; wiz.className='fb-meal-wizard';
    wiz.innerHTML='<div class="fb-ai-top"><div><span>Meal Plan AI</span><h3>Build your nutrition plan</h3><p>Step-by-step, clean and based on your saved profile where possible.</p></div><strong id="mealStepPill">1 / 5</strong></div><div class="fb-ai-progress"><i id="mealProgressFill" style="width:20%"></i></div><section class="fb-ai-step active" data-step="1"><h4>Goal</h4><div class="fb-meal-options"><button data-field="goal" data-value="fatloss"><strong>Lose fat</strong><small>High protein and controlled calories.</small></button><button data-field="goal" data-value="muscle"><strong>Build muscle</strong><small>Fuel growth and performance.</small></button><button data-field="goal" data-value="maintenance"><strong>Maintain</strong><small>Balanced clean eating.</small></button></div></section><section class="fb-ai-step" data-step="2"><h4>Your details</h4><div class="fb-ai-profile-card"><div><strong>'+esc(u.name||'Profile info')+'</strong><span>'+esc((u.age||'No age')+' years • '+(u.weight||'No weight')+' kg • '+(u.height||'No height')+' cm')+'</span></div><a href="profile.html">Change information</a></div><div class="fb-meal-mini-form"><input id="mwAge" type="number" placeholder="Age" value="'+esc(u.age||'')+'"><select id="mwGender"><option value="">Gender</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select><input id="mwWeight" type="number" placeholder="Weight kg" value="'+esc(u.weight||'')+'"><input id="mwHeight" type="number" placeholder="Height cm" value="'+esc(u.height||'')+'"></div><div id="mwAgeWarning" class="fb-age-warning">Minimum age is 16.</div></section><section class="fb-ai-step" data-step="3"><h4>Food style</h4><div class="fb-meal-options"><button data-field="diet" data-value="normal"><strong>Balanced</strong><small>Flexible simple meals.</small></button><button data-field="diet" data-value="highprotein"><strong>High protein</strong><small>Protein-focused meals.</small></button><button data-field="diet" data-value="budget"><strong>Budget friendly</strong><small>Cheaper foods.</small></button><button data-field="diet" data-value="easy"><strong>Fast meals</strong><small>Low cooking time.</small></button><button data-field="diet" data-value="vegetarian"><strong>Vegetarian</strong><small>No meat.</small></button></div></section><section class="fb-ai-step" data-step="4"><h4>Meal structure</h4><div class="fb-meal-options"><button data-field="meals" data-value="3"><strong>3 meals</strong><small>Simple and minimal.</small></button><button data-field="meals" data-value="4"><strong>4 meals</strong><small>Balanced daily rhythm.</small></button><button data-field="meals" data-value="5"><strong>5 meals</strong><small>Structured plan.</small></button></div></section><section class="fb-ai-step" data-step="5"><h4>Final details</h4><div class="fb-meal-mini-form"><select id="mwTraining"><option value="3">2-3 training days</option><option value="5">4-5 training days</option><option value="6">6+ training days</option><option value="0">0-1 training days</option></select><select id="mwStyle"><option value="balanced">Balanced</option><option value="lowcalorie">Low calorie</option><option value="bulking">Bulking</option><option value="simple">Very simple</option></select><input id="mwAvoid" class="wide" placeholder="Foods to avoid / allergies" value="'+esc(u.allergies||'')+'"></div><button id="mwGenerate" class="fb-ai-generate">Generate 7-day meal plan</button></section><div class="fb-ai-actions"><button id="mwBack">Back</button><button id="mwNext">Next</button></div>';
    gen.prepend(wiz); if($('mwGender')) $('mwGender').value=u.gender||''; if($('mwTraining')) $('mwTraining').value=u.trainingDays||'3';
    let step=1,state={goal:'fatloss',diet:'normal',meals:'4'};
    function render(){ $$('#fitbrandMealWizardV18 .fb-ai-step').forEach(s=>s.classList.toggle('active',Number(s.dataset.step)===step)); $('mealStepPill').textContent=step+' / 5'; $('mealProgressFill').style.width=(step/5*100)+'%'; $('mwBack').style.visibility=step===1?'hidden':'visible'; $('mwNext').style.display=step===5?'none':'inline-flex'; sync(); }
    ...
    wiz.onclick=e=>{const b=e.target.closest('button'); if(!b)return; if(b.dataset.field){state[b.dataset.field]=b.dataset.value; $$('[data-field="'+b.dataset.field+'"]').forEach(x=>x.classList.toggle('selected',x===b)); sync()} if(b.id==='mwBack'&&step>1){step--;render()} if(b.id==='mwNext'&&step<5){if(step===2&&!validateAge($('mwAge').value)){$('mwAgeWarning')?.classList.add('show');ageAlert();return}step++;render()} if(b.id==='mwGenerate'){if(!validateAge($('mwAge').value)){ageAlert();return}sync(); if(typeof window.generateMealPlan==='function') window.generateMealPlan();} };
    $$('[data-field="goal"]')[0]?.classList.add('selected'); $$('[data-field="diet"]')[0]?.classList.add('selected'); $$('[data-field="meals"]')[1]?.classList.add('selected'); render();
  }
  function patchBundle(){
    const b=document.querySelector('.fitbrand-bundle-offer'); if(!b) return;
    if(!b.querySelector('.fb-bundle-micro')) b.insertAdjacentHTML('beforeend','<div class="fb-bundle-micro"><span>All 3 training programs</span><span>Meal Plan Guide AI included</span><span>Training generator access</span><span>Best value</span></div>');
  }
  function boot(){
    ensureProfile(); updateProfileUI(); updateCartCount(); enhanceCheckout(); processConfirmation(); makeComingSoon(); renderAccountPages(); gatePremiumTools(); patchBundle();
    if(getUser()&&(purchases().includes('mealplan')||purchases().includes('bundle'))&&$('mealGenerator')) buildMealWizard();
    showWelcome();
    $$('.cart-icon-btn').forEach(btn=>btn.onclick=e=>{e.preventDefault();openCartDrawer()});
    document.addEventListener('click',e=>{const m=$('profileMenu'),b=document.querySelector('.profile-icon-btn'); if(m&&b&&!m.contains(e.target)&&!b.contains(e.target)) m.classList.remove('show')});
    setInterval(()=>{updateProfileUI();gatePremiumTools();},1500);
  }
  Object.assign(window,{getFitBrandUser:getUser,saveFitBrandUser:saveUser,logoutFitBrandUser:logout,updateFitBrandProfileUI:updateProfileUI,updateProfileUI,toggleProfileMenu,openProfileModal,closeProfileModal,loginFitBrandUser,addToCart,updateCartCount,removeDrawerItem,openCartDrawer,closeCartDrawer,handleMealPreviewClick,openGeneratorModal:openTrainingGenerator,closeGeneratorModal,generateModalPlan:openTrainingGenerator});
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
})();
