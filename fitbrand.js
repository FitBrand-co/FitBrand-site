  ...
  function renderAccountPages(){
    ...
    if($('accessGrid')){
      ...
      $('accessGrid').innerHTML = ['aesthetic','shred','strength','bundle','mealplan','belt','straps'].map(key => {
        ...
        return `<article class="access-card ${has?'owned':''}"><div><span>${has?'Unlocked':'Locked'}</span><h3>${escapeHtml(p.name)}</h3><p>${has?'You have access on this device.':'Request access to unlock this product.'}</p></div><a class="${has?'btn-dark':'btn-outline'}" href="${has?p.open:p.buy}">${has?'Open':'Request access'}</a></article>`;
      }).join('');
    }
    if($('ordersList')){
...
(function(){
  ...
  function handleMealPreviewClick(){if(!user()){sessionStorage.setItem('fitbrandPendingMealOpen','1');openProfileModal('login');return}if(!(purchases().includes('mealplan')||purchases().includes('bundle'))){let b=document.querySelector('a[href="purchase-access.html?product=mealplan"]');b?.scrollIntoView({behavior:'smooth',block:'center'});notice('Request access to unlock this product');return}let box=$('meal-plan-ai');if(box){box.classList.add('unlocked');buildMealWizard();box.scrollIntoView({behavior:'smooth',block:'start'})}}
  ...
  function renderAccess(){if(!$('accessGrid'))return;let own=purchases();$('accessGrid').innerHTML=['aesthetic','shred','strength','bundle','mealplan'].map(k=>{let p=PRODUCTS[k],has=own.includes(k);return '<article class="access-card '+(has?'owned':'')+'"><div><span>'+(has?'Unlocked':'Locked')+'</span><h3>'+esc(p[0])+'</h3><p>'+(has?'You have access on this device.':'Request access to unlock this product.')+'</p></div><a class="'+(has?'btn-dark':'btn-outline')+'" href="'+(has?p[2]:'purchase-access.html?product='+k)+'">'+(has?'Open':'Request access')+'</a></article>'}).join('')}
...
(function(){
  ...
  function renderAccessIfPresent(){
    ...
    const own=purchases();
    grid.innerHTML=Object.keys(productMap).map(k=>{const has=own.includes(k);const p=productMap[k];return '<article class="access-card '+(has?'owned':'')+'"><div><span>'+(has?'Unlocked':'Locked')+'</span><h3>'+p[0]+'</h3><p>'+(has?'You have access on this logged-in profile/device.':(logged?'Request access to unlock this product.':'Log in to view saved access.'))+'</p></div><a class="'+(has?'btn-dark':'btn-outline')+'" href="'+(has?p[1]:(logged?'purchase-access.html?product='+k:'#'))+'">'+(has?'Open':(logged?'Request access':'Log in required'))+'</a></article>'}).join('');
  }
  ...
  function patchDigitalBuyLinks(){
    document.querySelectorAll('a[href*="early-access.html?product="], a[href*="checkout.html?product="]').forEach(a => {
      ...
      if(DIGITAL.includes(key)) {
        ...
        if(/join|buy|start|early/i.test(a.textContent || '')) a.textContent = 'Request access';
      }
    });
