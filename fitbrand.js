...
(function(){
  ...
  const PRODUCTS = {
    aesthetic:{name:'Aesthetic Program Monthly',price:9.99,type:'digital',buy:'early-access.html?interest=aesthetic',open:'index.html?purchased=aesthetic'},
    shred:{name:'Shred Program Monthly',price:9.99,type:'digital',buy:'early-access.html?interest=shred',open:'index.html?purchased=shred'},
    strength:{name:'Strength Program Monthly',price:9.99,type:'digital',buy:'early-access.html?interest=strength',open:'index.html?purchased=strength'},
    bundle:{name:'Complete Bundle + Meal Plan AI Monthly',price:24.99,type:'digital',buy:'early-access.html?interest=bundle',open:'index.html?purchased=bundle'},
    mealplan:{name:'Meal Plan Guide AI Monthly',price:7.99,type:'digital',buy:'early-access.html?interest=mealplan',open:'recommended.html?purchased=mealplan#meal-plan-ai'},
  ...
  function renderAccountPages(){
    ...
    if($('accessGrid')){
      ...
      $('accessGrid').innerHTML = ['aesthetic','shred','strength','bundle','mealplan','belt','straps'].map(key => {
        ...
        return `<article class="access-card ${has?'owned':''}"><div><span>${has?'Unlocked':'Locked'}</span><h3>${escapeHtml(p.name)}</h3><p>${has?'You have access on this device.':'Join early access to be notified at launch.'}</p></div><a class="${has?'btn-dark':'btn-outline'}" href="${has?p.open:p.buy}">${has?'Open':'Join early access'}</a></article>`;
      }).join('');
    }
    if($('ordersList')){
  ...
  function handleMealPreviewClick(){
    ...
    if(hasMealAccess()){
      ...
      const buy = document.querySelector('a[href="early-access.html?interest=mealplan"]');
...
(function(){
  ...
  function handleMealPreviewClick(){if(!user()){sessionStorage.setItem('fitbrandPendingMealOpen','1');openProfileModal('login');return}if(!(purchases().includes('mealplan')||purchases().includes('bundle'))){let b=document.querySelector('a[href="early-access.html?interest=mealplan"]');b?.scrollIntoView({behavior:'smooth',block:'center'});notice('Buy Meal Plan AI to unlock');return}let box=$('meal-plan-ai');if(box){box.classList.add('unlocked');buildMealWizard();box.scrollIntoView({behavior:'smooth',block:'start'})}}
  ...
  function renderAccess(){if(!$('accessGrid'))return;let own=purchases();$('accessGrid').innerHTML=['aesthetic','shred','strength','bundle','mealplan'].map(k=>{let p=PRODUCTS[k],has=own.includes(k);return '<article class="access-card '+(has?'owned':'')+'"><div><span>'+(has?'Unlocked':'Locked')+'</span><h3>'+esc(p[0])+'</h3><p>'+(has?'You have access on this device.':'Join early access to be notified at launch.')+'</p></div><a class="'+(has?'btn-dark':'btn-outline')+'" href="'+(has?p[2]:'early-access.html?interest='+k)+'">'+(has?'Open':'Join early access')+'</a></article>'}).join('')}
...
(function(){
  ...
  function renderAccessIfPresent(){
    ...
    const own=purchases();
    grid.innerHTML=Object.keys(productMap).map(k=>{const has=own.includes(k);const p=productMap[k];return '<article class="access-card '+(has?'owned':'')+'"><div><span>'+(has?'Unlocked':'Locked')+'</span><h3>'+p[0]+'</h3><p>'+(has?'You have access on this logged-in profile/device.':(logged?'Join early access to be notified at launch.':'Log in to view saved access.'))+'</p></div><a class="'+(has?'btn-dark':'btn-outline')+'" href="'+(has?p[1]:(logged?'early-access.html?interest='+k:'#'))+'">'+(has?'Open':(logged?'Join early access':'Log in required'))+'</a></article>'}).join('');
  }
